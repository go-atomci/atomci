...
import (
	"github.com/go-atomci/atomci/core/publishes"
...
func (p *PublishController) Create() {
	...
	req := &publishes.PublishReq{}
	p.DecodeJSONReq(req)
	pm := publishes.NewPublishManager()
...
func (p *PublishController) TriggerBackTo() {
	...
	stageID, _ := p.GetInt64FromPath(":stage_id")
	currentUser := p.User
	req := &publishes.TriggerBackToReq{}
	p.DecodeJSONReq(req)
	pm := publishes.NewPublishManager()
