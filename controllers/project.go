...
import (
	...
	"github.com/go-atomci/atomci/core/projects"
...
func (p *ProjectController) AppDelete() {
	...
	logs.Info("Delete application successfully!", "cluster: "+cluster+",", "namespace: "+namespace+",", "name: "+appname, "!")
...
func (p *ProjectController) AppRestart() {
	...
	logs.Info("Restart application successfully!", "cluster: "+cluster+",", "namespace: "+namespace+",", "name: "+appname, "!")
	p.Data["json"] = NewResult(true, nil, "")
	p.ServeJSON()
}
...
func (p *ProjectController) AppScale() {
	...
	logs.Info("scale application succefully,", "cluster: "+cluster+",", "namespace: "+namespace+",", "name: "+appname, "!")
	p.Data["json"] = NewResult(true, nil, "")
	p.ServeJSON()
}
...
func (p *ProjectController) CreateProjectEnv() {
	projectID, _ := p.GetInt64FromPath(":project_id")
	request := projects.ProjectEnvReq{}
	creator := p.User
...
func (p *ProjectController) UpdatePipelineConfig() {
	...
	request := projects.PipelineReq{}
	currentUser := p.User
	p.DecodeJSONReq(&request)
	mgr := projects.NewProjectManager()
...
func (p *ProjectController) UpdateProjectApp() {
	projectID, _ := p.GetInt64FromPath(":project_id")
	projectAppID, _ := p.GetInt64FromPath(":project_app_id")
	req := &projects.ProjectAppUpdateReq{}
	p.DecodeJSONReq(req)
	pm := projects.NewProjectManager()
...
func (p *ProjectController) SwitchProjectBranch() {
	projectID, _ := p.GetInt64FromPath(":project_id")
	projectAppID, _ := p.GetInt64FromPath(":project_app_id")
	req := &projects.ProjectAppBranchUpdateReq{}
	p.DecodeJSONReq(req)
	pm := projects.NewProjectManager()
