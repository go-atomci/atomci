<template>
  <div class="page-content">
    <div class="portlet-body mt10 min-height150">
      <template>
        <div class="pv10" v-show="proShow">
            ...
            <el-col :span="24">
              <div class="setTitle">
                <div class="f-r" v-if="projectInfo.operations">
                  <el-button
                    @click="$refs.mark.show(projectInfo)"
                    v-if="projectInfo.operations['manual']"
                    >人工卡点</el-button
                  >
                  <el-button
                    @click="$refs.merge.show(projectInfo)"
                    v-if="projectInfo.operations['merge-branch']"
                    >合并分支</el-button
                  >
                  <el-button
                    @click="
                      $refs.goToBuild.doShows(
                        projectInfo.id,
                        projectInfo.stage_id,
                        projectInfo.step
                      )
                    "
                    v-if="projectInfo.operations['build']"
                    >构建</el-button
                  >
                  <el-button
                    @click="$refs.deploy.doShows(projectInfo)"
                    v-if="projectInfo.operations['deploy']"
                    >部署</el-button
                  >
                  <el-button
                    @click="
                      termination(projectInfo.id, projectInfo.stage_id, projectInfo.step_type)
                    "
                    v-if="projectInfo.operations['terminate']"
                    >终止</el-button
                  >
                  <el-button
                    @click="$refs.nextstage.show(projectInfo.id, projectInfo.stage_id)"
                    v-if="projectInfo.operations['next-stage']"
                    >{{ $t('bm.add.enterNextStage') }}</el-button
                  >
                  <el-button
                    @click="$refs.backto.show(projectInfo.id, projectInfo.stage_id)"
                    v-if="projectInfo.operations['back-to']"
                    >{{ $t('bm.add.rollBack') }}</el-button
                  >
                  <el-button v-show="statusCheck" @click="gotoclose(projectInfo.id)">{{
                    $t('bm.deployCenter.achieved')
                  }}</el-button>
                  <el-button @click="$refs.publishEdit.doShows(projectInfo)">{{
                    $t('bm.other.edit')
                  }}</el-button>
                  <el-button type="danger" @click="gotodelete(projectInfo.id)">{{
                    $t('bm.other.delete')
                  }}</el-button>
                  <el-button type="el-button el-button--success is-plain" @click="getVersionInfo">{{
                    $t('bm.other.refresh')
                  }}</el-button>
                </div>
                {{ projectInfo.version_no }}<br /><span class="setDescription">{{
                  projectInfo.name
                }}</span>
              </div>
            </el-col>
          ...
          <el-row>
            <el-col class="setText"> 创建者：{{ projectInfo.creator }} </el-col>
            <el-col class="setText"> 创建时间：{{ projectInfo.create_at }} </el-col>
          </el-row>
          <el-row>
            <el-col class="setText"> 开始时间：{{ projectInfo.start_at }} </el-col>
    ...
    <div class="portlet-body mt10 pv20 min-height150" v-if="statusCheck">
      <div class="setTitle" style="text-align: -webkit-left">
        当前环境 (<span class="setDescription">{{ projectInfo.stage_name }}</span
        >)
      </div>
    ...
    <div class="portlet-body mt10 pv10">
      <el-tabs v-model="activeName">
        <el-tab-pane label="操作历史" name="history">
          <template>
            <el-table stripe :data="historyData" style="width: 100%">
              <el-table-column
                prop="creator"
        ...
        <el-tab-pane label="应用列表" name="appList">
          <p style="float: left" v-if="statusCheck">
            <el-button
              type="primary"
              @click="
                $refs.versionAdd.doShow(
                  $route.params.projectID,
                  $route.params.versionId,
                  projectInfo.version_no
                )
              "
              >添加应用</el-button
            >
          ...
          <el-table class="mt10" :data="projectInfo.apps">
            ...
            <el-table-column
              prop="language"
              :label="$t('bm.deployCenter.language')"
              sortable
              min-width="10%"
              :show-overflow-tooltip="true"
            />
            <el-table-column
              prop="branch_name"
              :label="$t('bm.deployCenter.releaseBran')"
              min-width="11%"
              :show-overflow-tooltip="true"
            />
            <el-table-column
              prop="compile_command"
              :label="$t('bm.deployCenter.buildCompile')"
              min-width="11%"
              :show-overflow-tooltip="true"
            />
            <el-table-column
              :label="$t('bm.deployCenter.operation')"
              min-width="10%"
              v-if="statusCheck"
            >
              <template slot-scope="scope">
...
export default {
  ...
  methods: {
      ...
      backend.getListdetail(this.$route.params.projectID, this.$route.params.versionId, (data) => {
        if (data) {
          ...
          data.end_at = data.end_at
            ? Utils.format(new Date(data.end_at), 'yyyy-MM-dd hh:mm:ss')
            : '';
          let statusName = '';
    ...
    termination(id, stargid, name) {
      MessageBox.confirm(this.$t('bm.add.sureStopPublishCodeModule'), this.$t('bm.add.hint'), {
        type: 'warning',
      })
        .then(() => {
          const params = {
    ...
    // 关闭-归档
    gotoclose(id) {
      MessageBox.confirm(this.$t('bm.add.isAgreeAchieved'), this.$t('bm.infrast.tips'), {
        type: 'warning',
      })
        .then(() => {
          const that = this;
    ...
    // 删除
    gotodelete(id) {
      MessageBox.confirm(this.$t('bm.add.isAgreeDelete'), this.$t('bm.infrast.tips'), {
        type: 'warning',
      })
        .then(() => {
          const that = this;
    ...
    // 移除应用
    removeApp(id) {
      MessageBox.confirm(this.$t('bm.add.sureDelete'), this.$t('bm.infrast.tips'), {
        type: 'warning',
      })
        .then(() => {
          backend.removeApp(
